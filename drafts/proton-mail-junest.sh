...
# EXTRA STEP
sed -i "s#Exec=#Exec=/usr/share/$APP/#g" ./"$APP".AppDir/*.desktop
cd ./"$APP".AppDir/.junest/opt/"$APP"
ln -s Proton\ Mail proton-mail
cd -
if test -d ./"$APP".AppDir/.junest/usr/share/"$APP"; then
	rm -Rf ./"$APP".AppDir/.junest/opt/"$APP"
else
	mv ./"$APP".AppDir/.junest/opt/"$APP" ./"$APP".AppDir/.junest/usr/share/"$APP"
fi
