...
LIBSAVED="libv4lconvert.so libmpg123 libvorbisfile.so libidn libunistring.so libhogweed.so libnettle.so libsodium.so libpgm \
libsoxr.so libjxl_cms libhwy.so libbrotlienc.so libogg.so libsharpyuv.so libmvec.so libxcb libGLX.so libgomp.so libFLAC.so libuuid $lib_audio_keywords $lib_browser_launcher"
